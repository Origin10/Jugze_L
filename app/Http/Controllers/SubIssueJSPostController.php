...
class SubIssueJSPostController extends Controller
{
    public function postThumbChange(Request $request){
        ...
        $targetComment = $firebase->get(Constant::FIREBASE_DEFAULT_PATH_SUB_ISSUE_COMMENT.
                $request->subIssueId.'/'.$request->messageId.'/');
        ...
        if($request->up == 'up'){
            $userSubIssue = UserSubIssue::where('user_id', $userId)
                ->where('sub_issue_id', $request->subIssueId)->first();
            if($userSubIssue->status == Constant::USER_ISSUE_STATUS_POS){
                ...
                $firebase->set(Constant::FIREBASE_DEFAULT_PATH_SUB_ISSUE_COMMENT.
                    $request->subIssueId.'/'.$request->messageId.'/thumb_up_pos/'
                ...
                $currentThumbs->$userId = true;
                $firebase->set(Constant::FIREBASE_DEFAULT_PATH_SUB_ISSUE_COMMENT.
                    $request->subIssueId.'/'.$request->messageId.'/thumb_up_neg/'
                    , \json_encode($currentThumbs));
            }
            $userStatus = $userSubIssue->status;
        }
        else{
            if(isset($targetComment->thumb_up_pos)){
    ...
    public function postDeleteComment(Request $request){
        ...
        $firebase = new FirebaseLib(Constant::getFirebaseDefaultURL(), Constant::FIREBASE_DEFAULT_TOKEN);
        $firebase->set(Constant::FIREBASE_DEFAULT_PATH_SUB_ISSUE_COMMENT.
                    $request->subIssueId.'/'.$request->messageId.'/delete/'
                    , true);
    ...
    public function postEditComment(Request $request){
        ...
        $firebase = new FirebaseLib(Constant::getFirebaseDefaultURL(), Constant::FIREBASE_DEFAULT_TOKEN);
        $firebase->set(Constant::FIREBASE_DEFAULT_PATH_SUB_ISSUE_COMMENT.
            $request->subIssueId.'/'.$request->messageId.'/comment/'
            , $request->content);
    ...
    public function postJudge(Request $request){
        ...
        $subIssue = SubIssue::find($request->subIssueId);

        $posJudgesTemp = UserIssue::where('issue_id', $subIssue->issue_id)
            ->where('judge', Constant::USER_ISSUE_STATUS_POS)
            ->get();
        $posJudges = array();
        foreach($posJudgesTemp as $temp){
            if(count(UserSubIssue::where('user_id', $temp->user_id)->where('sub_issue_id', $subIssue->id)->get()) > 0){
                array_push($posJudges, $temp);
            }
        }

        $negJudgesTemp = UserIssue::where('issue_id', $subIssue->issue_id)
            ->where('judge', Constant::USER_ISSUE_STATUS_NEG)
            ->get();
        $negJudges = array();
        foreach($negJudgesTemp as $temp){
            if(count(UserSubIssue::where('user_id', $temp->user_id)->where('sub_issue_id', $subIssue->id)->get()) > 0){
                array_push($negJudges, $temp);
            }
        }
        ...
        if(isset($judges)){
            $firebase = new FirebaseLib(Constant::getFirebaseDefaultURL(), Constant::FIREBASE_DEFAULT_TOKEN);
            $firebase->set(Constant::FIREBASE_DEFAULT_PATH_SUB_ISSUE_JUDGE.
                    $request->subIssueId.'/'.
                    $request->messageId
                , $judges);
        }
    ...
    public function postJudgeReply(Request $request){
        $firebase = new FirebaseLib(Constant::getFirebaseDefaultURL(), Constant::FIREBASE_DEFAULT_TOKEN);
        $targetComment = $firebase->get(Constant::FIREBASE_DEFAULT_PATH_SUB_ISSUE_JUDGE.
                $request->subIssueId.'/'.$request->messageId.'/');
        $targetComment = \json_decode($targetComment);
        ...
        foreach($targetComment as $userId => $reply){
            if($userId == 'user_'.$request->userId){
                $firebase->set(Constant::FIREBASE_DEFAULT_PATH_SUB_ISSUE_JUDGE.
                    $request->subIssueId.'/'.$request->messageId.'/'.$userId
                , $request->keep);
        ...
        if($resultDelete >= $gap){
            //刪檢舉區
            $firebase->delete(Constant::FIREBASE_DEFAULT_PATH_SUB_ISSUE_JUDGE.
                $request->subIssueId.'/'.$request->messageId.'/');
            //刪留言
            $firebase->set(Constant::FIREBASE_DEFAULT_PATH_SUB_ISSUE_COMMENT.
                    $request->subIssueId.'/'.$request->messageId.'/delete/'
    ...
    public function postTagFloor(Request $request){
        ...
        if(strcmp(config('app.env'), 'local') == 0){
            ...
            Mail::to($taggedUser->email)->send(new CommentTag($request->subIssueTitle, $request->floor, $request->subIssueUrl));
            // Mail::to("demo.or.test.0823@gmail.com")->send(new CommentTag($request->subIssueTitle, $request->floor, $request->subIssueUrl));
            return response()->json(['status' => true]);
        }
    }
}
